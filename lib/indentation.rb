...
module Indentation 
  def indent(func)     
    ...
    cur_indent,line1_space, line2_space= 0,0,0
    res_key(func)
    $i = 0
    until   (func.size) == $i
      $i += 1  until !func[$i].empty?
     obs_indent = (line2_space - line1_space)
      if   obs_indent !=  cur_indent
       ...
       line1_space,line2_space = 0,0
      func[$i-1].each_char{|c| c ==' ' ? line1_space +=1 : break}
      func[$i].each_char{|c| c ==' ' ? line2_space +=1 : break} unless func.size  - 1 == $i 
      cur_indent = indent_c(func)
     $i += 1
    end
  end 
  def indent_c(func)
    arr,arr2 = %w(def class if unless until case while begin module do),%w(else end elsif) 
